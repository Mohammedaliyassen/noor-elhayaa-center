...
const AdminUsersSection = () => {
  ...
  const fetchUsers = async () => {
    ...
    if (!roles?.length) { setUsers([]); setLoading(false); return; }
    ...
    <Card>
      <CardContent className="p-0">
        {loading ? (
          ...
          <p className="p-8 text-center text-muted-foreground">{isRTL ? "لا يوجد مستخدمين" : "No users"}</p>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {list.map((u) => (
                <TableRow key={u.user_id}>
                  ...
                  <TableCell>
                    <Badge variant={u.role === "doctor" ? "default" : "secondary"}>
                      {u.role === "doctor" ? (isRTL ? "طبيب" : "Doctor") : (isRTL ? "مريض" : "Patient")}
                    </Badge>
                  </TableCell>
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-2xl font-bold">{isRTL ? "إدارة المستخدمين" : "User Management"}</h2>
      <Tabs defaultValue="doctors">
        <TabsList>
          <TabsTrigger value="doctors">{isRTL ? `الأطباء (${doctors.length})` : `Doctors (${doctors.length})`}</TabsTrigger>
          <TabsTrigger value="patients">{isRTL ? `المرضى (${patients.length})` : `Patients (${patients.length})`}</TabsTrigger>
        </TabsList>
