...
import {
  Users, FileText, Calendar, Tag, LogOut, Menu, X, Globe, Home,
  LayoutDashboard, Shield, UserCog, MessageCircle,
} from "lucide-react";
import SEOHead from "@/components/SEOHead";
import NotificationBell from "@/components/NotificationBell";
import AdminOverview from "./sections/AdminOverview";
import AdminArticlesSection from "./sections/AdminArticlesSection";
import AdminOffersSection from "./sections/AdminOffersSection";
import AdminUsersSection from "./sections/AdminUsersSection";
import AppointmentsSection from "./sections/AppointmentsSection";
import ChatSection from "@/components/chat/ChatSection";

type Tab = "overview" | "users" | "articles" | "appointments" | "offers" | "messages";
...
const AdminDashboard = () => {
  ...
  const tabs: { key: Tab; label: string; icon: React.ElementType }[] = [
    ...
    { key: "users", label: isRTL ? "المستخدمين" : "Users", icon: UserCog },
    { key: "articles", label: isRTL ? "المقالات" : "Articles", icon: FileText },
    { key: "appointments", label: isRTL ? "المواعيد" : "Appointments", icon: Calendar },
    { key: "offers", label: isRTL ? "العروض" : "Offers", icon: Tag },
    { key: "messages", label: isRTL ? "الرسائل" : "Messages", icon: MessageCircle },
  ];
  ...
  const renderContent = () => {
    switch (tab) {
      ...
      case "articles": return <AdminArticlesSection />;
      case "appointments": return <AppointmentsSection isAdmin />;
      case "offers": return <AdminOffersSection />;
    ...
    <div className="flex min-h-screen bg-background">
      ...
      >
          ...
          {tabs.map(({ key, label, icon: Icon }) => {
            ...
            return (
              ...
              >
                ...
                {showUnreadBadge && (
                  <Badge variant="default" className="min-w-5 justify-center px-1.5 py-0 text-[10px]">
        ...
        <div className="border-t border-border p-3 space-y-1">
          ...
          <button onClick={() => setLanguage(language === "ar" ? "en" : "ar")} className="flex w-full items-center gap-3 rounded-lg px-3 py-2 text-sm text-muted-foreground hover:bg-muted">
      ...
      <div className="flex flex-1 flex-col">
        <header className="sticky top-0 z-20 flex h-16 items-center gap-3 border-b border-border bg-background/80 px-4 backdrop-blur-md">
          ...
          <h1 className="flex-1 text-lg font-semibold text-foreground">
            {isRTL ? `مرحبًا، ${displayName || "المدير"}` : `Welcome, ${displayName || "Admin"}`}
          </h1>
          <NotificationBell />
        </header>
